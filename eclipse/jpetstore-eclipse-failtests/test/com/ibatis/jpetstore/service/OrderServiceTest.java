...
public class OrderServiceTest extends MockObjectTestCase {
	...
	public void testShouldCallGetOrderOnOrderDao() {
		...
		orderDao.expects(once()).method("getOrder").with(NOT_NULL).will(
		...
		daoManager.expects(once()).method("commitTransaction")
				.withNoArguments();

		daoManager.expects(once()).method("endTransaction").withNoArguments();

		OrderService service = new OrderService(
				(DaoManager) daoManager.proxy(), null, (OrderDao) orderDao
						.proxy(), null);
		service.getOrder(1);
	...
	public void testShouldCallGetOrdersByUsernameOnOrderDao() {
		Mock orderDao = mock(OrderDao.class);
		orderDao.expects(once()).method("getOrdersByUsername").with(NOT_NULL)
				.will(returnValue(new PaginatedArrayList(5)));
		OrderService service = new OrderService(null, null, (OrderDao) orderDao
				.proxy(), null);
	...
	public void testShouldCallInsertOrderOnOrderDao() {
		...
		seqDao.expects(once()).method("getNextId").with(NOT_NULL).will(
				returnValue(1));
		...
		itemDao.expects(once()).method("updateAllQuantitiesFromOrder").with(
				NOT_NULL);
		...
		OrderService service = new OrderService(
				(DaoManager) daoManager.proxy(), (ItemDao) itemDao.proxy(),
	...
	public void testOrderServiceForcedFailure1() {
		fail("Forced failure 1");
	}
	
	public void testOrderServiceForcedFailure2() {
		fail("Forced failure 2");
	}
	
	public void testOrderServiceForcedFailure3() {
		fail("Forced failure 3");
	}
